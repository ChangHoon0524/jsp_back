...
<html lang="ko">
  <head>
    ...
    <title>글 보기</title>
    <link rel="stylesheet" href="${pageContext.request.contextPath}/assets/css/board/boardRead.css" />
  </head>
  <body>
      ...
      <div class="view-wrap">
        ...
        <div class="view-attach">
           <c:forEach var="file" items="${board.getFiles() }" >
              <div class="img-box">
                 <img src="${pageContext.request.contextPath}/upload/${file.getFileSystemName()}" />
                 
                 <!-- 다운로드 버튼 -->
                 <!--  다운로드 받기 위해서는 시스템 이름이 필요하고 사용자에게 파일을 줄 때는 오리지널 네임으로 줘야한다 -->
                 <a href="${pageContext.request.contextPath}/file/download.file?fileSystemName="${file.getFileOriginalName()}">
        ...
        <div class="btn-group">
            <!-- 각 버튼 처리 경로 js로 수정하기 -->
            <button type="button" class="list-btn" data-boardNumber="${board.getBoardNumber() }"
    ...
    <script src="${pageContext.request.contextPath}/assets/js/board/boardRead.js"></script>
  </body>
</html>
